...
import { createTaskAction, updateTaskStatusAction } from "@/lib/actions/operations";
...
export default async function TasksPage({ searchParams }: TasksPageProps) {
  const [options, tasks, params] = await Promise.all([getOperationFormOptions(), getTasks(), searchParams]);
      ...
      <section className="grid gap-5 xl:grid-cols-[0.85fr_1.4fr]">
        <Card className="rounded-[2rem] border-border/80 bg-card/80">
          <CardHeader>
            ...
            <CardDescription>Limpieza, mantenimiento, inspecciones o acciones internas del equipo.</CardDescription>
          </CardHeader>
          <CardContent>
            <form action={createTaskAction} className="grid gap-4">
              ...
              <div className="grid gap-2">
                ...
                <Input id="title" name="title" required placeholder="Ej. Preparar check-in autonomo" />
              </div>

              <div className="grid gap-3 md:grid-cols-3">
                <div className="grid gap-2">
                  <Label htmlFor="type">Tipo</Label>
                  <select id="type" name="type" className="h-9 rounded-xl border border-input bg-background px-3 text-sm">
                    {taskTypes.map((type) => (
                ...
                <div className="grid gap-2">
                  ...
                  <select id="priority" name="priority" className="h-9 rounded-xl border border-input bg-background px-3 text-sm">
              ...
              <Textarea name="description" placeholder="Checklist, instrucciones o contexto para quien la ejecute." />
              <Button type="submit" className="rounded-full">Crear tarea</Button>
            </form>
          </CardContent>
        </Card>
        ...
        {tasks.length ? (
          <section className="grid auto-rows-fr gap-4 md:grid-cols-2">
            {tasks.map((task) => (
              <Card key={task.id} className="h-full rounded-[2rem] border-border/80 bg-card/80">
                <CardContent className="flex h-full flex-col p-5">
                  ...
                  <div className="mt-auto pt-8">
                    ...
                    <div className="mt-4 flex items-center gap-2">
                      ...
                      <form action={updateTaskStatusAction} className="ml-auto flex gap-2">
                        <input type="hidden" name="taskId" value={task.id} />
                        <select name="status" className="h-8 rounded-xl border border-input bg-background px-2 text-xs">
                        ...
                        <Button type="submit" size="sm" variant="outline" className="rounded-full">Guardar</Button>
                      </form>
                    </div>
                  </div>
                </CardContent>
